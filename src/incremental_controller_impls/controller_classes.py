...
from common.box_constraint_utils import combine_box
...
class BaseGPMPC_sanity:
    ...
    def init_opti_means_n_params(self):
        if self.integration_method in ["euler", "exact"]:
            opti = cs.Opti()
        elif self.integration_method == "ltv":
            opti = cs.Opti('conic')
        else:
            raise NotImplementedError
        self.opti = opti
...
class GPMPC_BnC(BaseGPMPC_sanity):
    def __init__(self, gp_fns, regions, region_subset_lim_lb, region_subset_lim_ub, delta_input_mask, gp_input_mask, Bd,
        ...
        self.env_model = kwargs.get('env_model', None)

        self.regions = regions
        self.num_regions = kwargs.get('num_regions', None)
    ...
    def setup_hybrid_means_manual_preds(self, gp_inp_vec_size, gp_input):
        if not (self.Bd == 0).all():
            ...
            res_compute_fns = [lambda x: [0] for r in range(self.num_regions)]
            mu_d_pw_arr = [[0 for _ in range(self.num_regions)] for _ in range(self.N)]
    ...
    def compute_mu_d_k(self, k, region_deltas_k, mu_d_arr=None, mu_d_pw_arr=None):
        ...
        # Alternate:
        # print("PRINTING ERROR SHAPES")
        # print(mu_d_pw_arr[k].shape)
        # print(region_deltas_k.shape)
        # print(mu_d_arr[:, k].shape)
    ...
    def define_delta_vars(self):
        region_deltas = self.opti.parameter(self.num_regions, self.N + 1)
    ...
    def generate_hld_for_warmstart(self):
        ...
        x_desired_clipped = self.clip_x_des() + 1e-7
        # print(x_desired_clipped)
        ...
        for sim_step in range(num_steps):
            ...
            for region_idx, region in enumerate(self.regions):
                self.hld_arr[region_idx, sim_step] = 1 if region.check_satisfaction(
                    (self.delta_input_mask @ joint_vec).T).item() is True else 0
            # print((self.delta_input_mask @ joint_vec).T)
            assert np.sum(self.hld_arr[:,
                          sim_step]) == 1, "Delta assignment %s is not unique for the given input %s. Please check the input and try again." % (self.hld_arr[:,sim_step], self.delta_input_mask @ joint_vec)
    ...
    def fine_simulate(self, clip_to_region_fn, clip_state, fine_disc_time, start_state, mu_u_0,
                      no_noise, dt_dyn_nom, **kwargs):
        ops = fwdsim_w_pw_res(clip_to_region_fn=clip_to_region_fn, clip_state=clip_state,
                              ...
                              integration_method=self.integration_method, dt_dyn_nom=dt_dyn_nom,
                              **kwargs)  # Don't include noise for this increment since not shrinking based on variance outputs.
        return ops

    def coarse_simulate(self, mu_u_0, no_noise, dt_dyn_nom, **kwargs):
    ...
    def perform_forward_sim(self, mu_x_0, mu_u_0, mu_x_1, no_noise=False):
        ...
        elif self.fwd_sim == "w_pw_res" and self.env_model is not None:
            sampled_ns = self.env_model.fwd_sim(uk=mu_u_0)
            ...
            if fine_simulate:
                ...
                for i in range(num_integrations):
                    ...
                    if i == num_integrations - 1:
                        ...
                        clip_state = False
                    ops = self.fine_simulate(clip_to_region_fn, clip_state, fine_disc_time,
                                             start_state, mu_u_0, no_noise, dt_dyn_nom)
                ...
                ops = self.coarse_simulate(mu_u_0, no_noise, dt_dyn_nom)
                if self.collision_check:
...
class GPMPC_D(GPMPC_BnC):
            ...
            for k in range(self.N):
                ...
                if self.skip_feedback:
                    ...
                    if self.gp_fns is not None:
                        ...
                        Sigma_x_k_plus_1 = np.zeros((self.n_x, self.n_x))
    ...
    def shrunk_gen_commons(self, simulation_length):
        ...
        if self.gp_fns is not None:
            ...
            return [], [0 for k in range(self.N + simulation_length - 1)]
...
class GPMPC_fixeddelta(GPMPC_D):
    def __init__(self, satisfaction_prob, gp_fns, regions, region_subset_lim_lb, region_subset_lim_ub, delta_input_mask,
                 gp_input_mask, Bd, sys_dyn, Q, R, horizon_length, X: box_constraint, U: box_constraint, n_x, n_u, n_d,
                 terminal_calc_fn="linearized_lqr", solver='ipopt', addn_solver_opts=None, add_delta_constraints=False,
                 ignore_init_constr_check=False, add_scaling=False, sampling_time=20 * 10e-3, state_plot_idxs=None,
                 integration_method='euler', ignore_callback=False, fwd_sim="nom_dyn", infeas_debug_callback=False,
                 skip_shrinking=False, skip_feedback=True, shrinking_approach="shrink_by_desired", K=None,
                 periodic_callback_freq=None, boole_deno_override=False, **kwargs):
    ...
    def define_delta_vars(self):
        self.region_delta = self.opti.parameter(self.num_regions, 1)  # only 1 delta vector across the horizon
        region_deltas = cs.repmat(self.region_delta, 1, self.N+1)
        return region_deltas

    def hld_init(self, sim_step=None):
        if sim_step is None:
            self.opti.set_value(self.region_delta, self.hld_arr[:, [0]])
        else:
            print("delta init", self.hld_arr[:, [sim_step]])
            self.opti.set_value(self.region_delta, self.hld_arr[:, [sim_step]])
...
class GPMPC_That(GPMPC_D):
    def __init__(self, That_predictor, satisfaction_prob, gp_fns, box_env_inst: Box_Environment, region_subset_lim_lb, region_subset_lim_ub, delta_input_mask,
                 gp_input_mask, Bd, sys_dyn, Q, R, horizon_length, X: box_constraint, U: box_constraint, n_x, n_u, n_d,
                 mapping_type='scratch_full_ds', initial_mapping_ds: Mapping_DS=None, siren_mapping=False,
                 add_delta_constraints=True, terminal_calc_fn="linearized_lqr", solver='ipopt', addn_solver_opts=None,
                 ignore_init_constr_check=False, add_scaling=False, sampling_time=20 * 10e-3, state_plot_idxs=None,
                 integration_method='euler', ignore_callback=False, fwd_sim="nom_dyn", infeas_debug_callback=False,
                 skip_shrinking=False, skip_feedback=True, shrinking_approach="shrink_by_desired", K=None, periodic_callback_freq=None, fixed_delta=False,
                 predictor_type="mess", **kwargs):
        self.That_predictor = That_predictor
        self.predictor_type = predictor_type
        self.mapping_type = mapping_type
        self.box_env_inst = box_env_inst
        self.fixed_delta = fixed_delta
        regions = box_env_inst.boxes
        self.siren_mapping = siren_mapping
        if self.mapping_type == 'scratch_full_ds':
            ...
            raise NotImplementedError
        self.num_regions = self.box_env_inst.num_surf
        kwargs["num_regions"] = self.num_regions
        super().__init__(satisfaction_prob, gp_fns, regions, region_subset_lim_lb, region_subset_lim_ub,
                         delta_input_mask, gp_input_mask, Bd, sys_dyn, Q, R, horizon_length, X, U, n_x, n_u, n_d,
                         add_delta_constraints, terminal_calc_fn, solver, addn_solver_opts, ignore_init_constr_check,
                         add_scaling, sampling_time, state_plot_idxs, integration_method, ignore_callback, fwd_sim,
    ...
    def generate_hld_for_warmstart(self):
        ...
        if self.predictor_type == "mess":
            raise NotImplementedError
        else:
            test_outputs, softmaxd_op, hard_test_ops, categorical_arr = self.That_predictor.gen_pred_outputs(test_X=self.delta_input_mask @ np.vstack([x_desired_clipped, np.zeros([self.n_u, num_steps])]))
            self.hld_arr = hard_test_ops.numpy()
    ...
    def hld_init(self, sim_step=None):
        ...
        if not self.fixed_delta:
            hld_k_arr = self.hld_arr[:, sim_step + 1:sim_step + 1 + self.N + 1]
        else:
            hld_k_arr = np.tile(self.hld_arr[:, [sim_step]], (1, self.N+1))
        self.opti.set_value(self.opti_dict["hld"], hld_k_arr)
...
class GPMPC_MINLP(GPMPC_D):
    ...
    def define_delta_vars(self):
        region_deltas = self.opti.variable(self.num_regions, self.N + 1)
        return region_deltas
