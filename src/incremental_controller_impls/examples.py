...
from incremental_controller_impls.smpc_base import OptiDebugger, construct_config_opts, construct_config_opts_minlp
...
def sanity_check_controller(x_init, velocity_override=0.7, early_termination=True, minimal_print=True,
                            ...
                            box_env_gen_fn=test_box_env_3_box_3_surf, box_env_viz=True, simulation_length_override=None,
    ...
    if problem_setup_fn is quad_2d_sys_1d_inp_res:
        sys_config_dict, inst_override = problem_setup_fn(velocity_limit_override=velocity_override, box_env_inst=box_env_gen_fn(viz=False),
                                                          sampling_time=sampling_time, ret_inst=True, region_viz=box_env_viz)
        ...
        sys_config_dict, inst_override = planar_lti_1d_inp_res(u_max=velocity_override, box_env_inst=box_env_gen_fn(viz=False),
                                                               sampling_time=sampling_time, ret_inst=True, region_viz=box_env_viz)
        limiter_names = ["x1_threshold", "x2_threshold"]
    # elif problem_setup_fn is vehicle_1d_inp_res:
    #     assert box_env_gen_fn in [vehicle_test_box_singlesurf]
    #     sys_config_dict, inst_override = vehicle_1d_inp_res(box_env_inst=box_env_gen_fn(viz=False, mu_idx=SurfaceSelector.ASPHALT),
    #                                                         sampling_time=sampling_time, ret_inst=True, region_viz=box_env_viz)
    #     limiter_names = ["x_max", "y_max"]
    # else:
    ...
    sys_config_dict["regions"] = sys_config_dict["box_env_inst"].boxes
    ...
    # elif problem_setup_fn == vehicle_1d_inp_res:
    #     st_example_traj_inst = ST_Example_Traj(mu_idx=SurfaceSelector.ASPHALT, traj_type=ST_Example_Traj.SLC_Small)
    #     st_example_traj_inst.fwdsim_traj()
    #     st_example_traj_inst.pad_traj(simulation_length+N+3)
    #     x_desired, u_warmstart = st_example_traj_inst.x_traj[:5, :], st_example_traj_inst.u_traj
    else:
    ...
    if not closed_loop:
        ...
        if show_plot:
            # if problem_setup_fn == vehicle_1d_inp_res:
            #     x_lim_min, x_lim_max = inst_override.x_min-2, inst_override.x_max+2
            #     y_lim_min, y_lim_max = inst_override.y_min-2, inst_override.y_max+2
            #     print(x_lim_min, x_lim_max, y_lim_min, y_lim_max)
            # else:
            x_lim_min, x_lim_max = -0.05, sys_config_dict[limiter_names[0]]+0.1
            y_lim_min, y_lim_max = -0.05, sys_config_dict[limiter_names[1]]+0.1
            mu_x_cl, axes = controller_utils.plot_CL_opt_soln(waypoints_to_track=waypoints_to_track,
                                                              data_dict_cl=data_dict_cl, ret_mu_x_cl=True,
                                                              state_plot_idxs=sys_config_dict["state_plot_idxs"],
                                                              plot_ol_traj=plot_ol_traj, axes=None,
                                                              ax_xlim=(x_lim_min, x_lim_max),
                                                              ax_ylim=(y_lim_min, y_lim_max),
                                                              legend_loc='upper center', ignore_legend=True, ret_ax=True)
            if box_env_viz:
                box_env_inst.visualize_env(ax=axes[0], alpha=0.05)

            # if problem_setup_fn == vehicle_1d_inp_res:
            #     vehicle_traj_viz_inst = VehicleTrajViz(0, 0, None)
            #     mu_u_cl = [adjust_1d_to_2d(data_dict_cl_k["mu_u"][:, 0]) for data_dict_cl_k in data_dict_cl]
            #     mu_u_cl = np.hstack(mu_u_cl)[:, :-1]
            #     mu_x_cl = [adjust_1d_to_2d(data_dict_cl_k["mu_x"][:, 0]) for data_dict_cl_k in data_dict_cl]
            #     mu_x_cl = np.hstack(mu_x_cl)
            #     vehicle_traj_viz_inst.plot_traj(mu_x_cl, mu_u_cl, sampling_time, viz_xu=True, ref_u_traj=u_warmstart[:, :simulation_length-1])
...
def gpmpc_BnC_controller(gp_fns, x_init, velocity_override=0.7, early_termination=True, minimal_print=True,
                         ...
                         box_env_gen_fn=test_box_env_3_box_3_surf, box_env_viz=True,
                         infeas_debug_callback=False, collision_check=False, waypoint_arr=((7, 0), (7, 7), (0, 7), (0, 0)),
                         simulation_length_override=None,
...
def gpmpc_D_controller(gp_fns, x_init, satisfaction_prob=0.95, velocity_override=0.7, early_termination=True,
                       ...
                       Q_override=None, R_override=None, Bd_override=None, ret_config_dict=False, fixed_delta=False, env_model=None):
    if problem_setup_fn is quad_2d_sys_1d_inp_res:
        sys_config_dict, inst_override = problem_setup_fn(velocity_limit_override=velocity_override, box_env_inst=box_env_gen_fn(viz=False),
                                                          sampling_time=sampling_time, ret_inst=True, region_viz=box_env_viz)
        limiter_names = ["x_threshold", "z_threshold"]
    ...
    # if problem_setup_fn is not vehicle_1d_inp_res:
    #     true_ds_inst.box_env_inst = sys_config_dict["box_env_inst"]
    sys_config_dict["regions"] = sys_config_dict["box_env_inst"].boxes
    ...
    if Bd_override is not None:
        sys_config_dict["Bd"] = Bd_override
    sys_config_dict["env_model"] = env_model
    # print(sys_config_dict)
    lti = inst_override.lti
    if lti and integration_method == 'exact':
        if not inst_override.symbolic.exact_flag:
    ...
    #     st_example_traj_inst.fwdsim_traj()
    #     st_example_traj_inst.pad_traj(simulation_length+N+3)
    #     x_desired, u_desired = st_example_traj_inst.x_traj[:5, :], st_example_traj_inst.u_traj
    else:
        # Nominal MPC solution for warmstarting.
    ...
    if not fixed_delta:
        ...
        print("Using single delta")
        controller_inst = GPMPC_fixeddelta(**configs)
...
def gpmpc_QP_controller(gp_fns, x_init, satisfaction_prob=0.95, velocity_override=0.7, early_termination=True,
                        ...
                        ignore_callback=False, num_discrete=50, sampling_time=20 * 10e-3,
                        box_env_gen_fn=test_box_env_3_box_3_surf, box_env_viz=True,
                        add_delta_constraints=False, fwd_sim="nom_dyn", true_ds_inst=None, include_res_in_ctrl=False,
                        infeas_debug_callback=False, plot_shrunk_sep=False, waypoint_arr=((7, 0), (7, 7), (0, 7), (0, 0)), collision_check=False,
...
def gpmpc_MINLP_controller(gp_fns, x_init, satisfaction_prob=0.95, velocity_override=0.7, early_termination=True,
    ...
    print_level = 0 if minimal_print else 5
    opts = construct_config_opts_minlp(print_level=print_level, add_monitor=False, early_termination=early_termination, hsl_solver=False,
                                       test_no_lam_p=no_lam_p, hessian_approximation=hessian_approximation, jac_approx=jac_approx)
    fn_config_dict["addn_solver_opts"] = opts
