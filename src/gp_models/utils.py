...
class GP_Model(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood, test_ard=False):
        """
        ...
        self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel(ard_num_dims = self.train_x.shape[-1] if test_ard else None))
...
def train(model, likelihood_fn, optimizer, lossfn_callable, train_x, train_y, num_iter=400, verbose=True,
    ...
    if verbose:
        ...
        print('final loss: %s, lengthscale: %s noise variance (sigma**2): %.5f, noise std dev (sigma): %.5f' % (
            loss.item(), model.covar_module.base_kernel.lengthscale, model.likelihood.noise.item(), math.sqrt(model.likelihood.noise.item())))
...
def get_gpds_attrs(gp_ds, no_squeeze=False):
    ...
    return train_x, gp_ds.train_y.type(torch.FloatTensor), gp_ds.num_surf, gp_ds.output_dims
...
class Piecewise_GPR_Callback(GPR_Callback):
    def __init__(self, name, likelihood_fns, models, output_dim, input_dim, num_regions, opts={}):
        ...
        self.models = models
        self.output_dims = output_dim
        assert self.output_dims == 1  # Multivar_GP_model in controller_classes_multivar_n_inp.py will extend to cases where this is not True.
...
def trained_gp_viz_test(models, likelihoods, gp_ds: GP_DS, fineness_param=(21, 51), model_type='global'):
    colours = ['r', 'g', 'b', 'cyan']
    dims, num_regions = gp_ds.input_dims, gp_ds.num_surf
    ...
    colours = gp_ds.get_colours(1 if model_type == 'global' else gp_ds.num_surf)
