...
def construct_pw_gpr_cb_from_models(models, likelihoods, input_dims, output_dims, num_regions):
    ...
    piecewise_gp_wrapped = Piecewise_GPR_Callback('f', likelihoods, models,
                                                  output_dim=output_dims, input_dim=input_dims, num_regions=num_regions,
                                                  opts={"enable_fd": True})
    return piecewise_gp_wrapped
...
def gen_wrapped_pw_gp(ds_ndim_test, return_error_attrs, num_regions, num_iter=400,
    ...
    piecewise_gp_wrapped = construct_pw_gpr_cb_from_models(models, likelihoods,
                                                           input_dims=res_input_dim, output_dims=res_output_dim, num_regions=num_regions)

    if return_error_attrs:
        return piecewise_gp_wrapped, models, likelihoods, region_trained_covs
    elif ret_ds:
        return piecewise_gp_wrapped, models, likelihoods, ds_ndim_test
    else:
        return piecewise_gp_wrapped, models, likelihoods


def construct_gp_wrapped_piecewise(ds_inst_in: GP_DS, box_env_inst: Box_Environment, viz=True, fineness_param=(20, 20), verbose=True,
...
def train_global_n_pw(ds_inst_in, box_env_inst, viz=True, verbose=True, return_error_attrs=False,
                      seed=None, fineness_param=(20, 20), num_iter=50, true_only=False,
                      terminate_by_change=False, save_to_file=False, load_from_file=False, file_save_name=None, file_load_name=None):

    pw_ops = construct_gp_wrapped_piecewise(ds_inst_in, box_env_inst=box_env_inst, viz=viz, fineness_param=fineness_param, verbose=verbose,
